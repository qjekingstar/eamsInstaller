package com.ekingstar.eams.installer.panels.userinput;
...
import com.izforge.izpack.panels.userinput.MultipleFileInputField;
import com.izforge.izpack.panels.userinput.PasswordGroup;
import com.izforge.izpack.panels.userinput.PasswordUIElement;
import com.izforge.izpack.panels.userinput.RadioButtonUIElement;
import com.izforge.izpack.panels.userinput.UIElement;
import com.izforge.izpack.panels.userinput.UIElementType;
...
public class SelectAppContextXMLPanel extends UserInputPanel {
	private static final Logger logger = Logger.getLogger(SelectAppContextXMLPanel.class.getName());
	 ...
	 *            the platform-model matcher
	 */
	/*public SelectAppContextXMLPanel(Panel panel, InstallerFrame parent, GUIInstallData installData, 
			Resources resources, RulesEngine rules, PlatformModelMatcher matcher) {
	...
	private void addDirectoryField(IXMLElement field) {
		...
		FileInputField fileInput = new DirInputField(this, this.installData, true, set, size, validatorConfig, mustExist, create);
	...
	private void addMultipleFileField(IXMLElement field) {
		...
		if (element == null) {
			...
			if (visRows != null) {
				try {
					...
					logger.log(Level.WARNING, "Illegal value for 'visibleRows'", e);
				}
			}
			...
			if (prefX != null) {
				try {
					...
					logger.log(Level.WARNING, "Illegal value for 'prefX'", e);
				}
			}
			...
			if (prefY != null) {
				try {
					...
					logger.log(Level.WARNING, "Illegal value for 'prefY'", e);
				}
			}
		...
		MultipleFileInputField fileInputField = new MultipleFileInputField(parent, installData, false, set, size, validatorConfig, filter, filterdesc, createMultipleVariables, visibleRows, preferredX, preferredY, labelText);
	...
	private void addFileField(IXMLElement field) {
		...
		FileInputField fileInputField = new FileInputField(this, this.installData, false, set, size, validatorConfig, filter, filterdesc);
	...
	public void panelActivate() {
		...
		if (spec == null) {
			...
			// TODO: translate
			emitError("User input specification could not be found", "The specification for the user input panel " + (p != null ? p.getPanelId() : "") + " could not be found. Please contact the packager.");
			parent.skipPanel();
		}
	...
	/**
	 * Asks the panel to set its own XML installDataGUI that can be brought back
	 * for an automated installation process. Use it as a blackbox if your panel
	...
	public void makeXMLData(IXMLElement panelRoot) {
		...
		for (TextValuePair pair : entries) {
			// IZPACK-283: read the value from installData instead of panel
			// installDataGUI
	...
	private boolean readDirectoryField(UIElement field) {
		...
		try {
			...
			if (result) {
				this.installData.setVariable(field.getAssociatedVariable(), panel.getSelectedFile().getAbsolutePath());
				entries.add(new SelectAppContextXMLPanel.TextValuePair(field.getAssociatedVariable(), panel.getSelectedFile().getAbsolutePath()));
			}
		} catch (Exception e) {
			logger.log(Level.WARNING, e.getMessage(), e);
		}
	...
	private boolean readFileField(UIElement field) {
		...
		try {
			...
			if (result) {
				String selectFileName = input.getSelectedFile().getName().length() != 0 ? input.getSelectedFile().getAbsolutePath() : "";
			...
			if (result) {
				...
				if (input.isCreateMultipleVariables()) {
					...
					for (String file : files) {
						...
						entries.add(new SelectAppContextXMLPanel.TextValuePair(indexedVariableName.toString(), file));
					}
					...
					entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, buffer.toString()));
				}
			}
		} catch (Exception e) {
			logger.log(Level.WARNING, e.getMessage(), e);
	...
	private void readSpec() throws Exception {
		installData.setVariable("dataInstallMode", "CUSTOM_DB");
		...
		if (data.hasChildren()) {
			...
			for (IXMLElement specElement : specElements) {
				...
				if (((userInputPanelSpecId != null) && (panelid != null) && (panelid.equals(userInputPanelSpecId)))) {
					List<IXMLElement> fieldElements = specElement.getChildrenNamed(FIELD_NODE_ID);
					loopField: for (IXMLElement fieldElement : fieldElements) {
						List<IXMLElement> specEles = fieldElement.getChildrenNamed(SPEC);
						for (IXMLElement specEle : specEles) {
							String specId = specEle.getAttribute("id");
							if ((userInputPanelSpecId + "." + "appContextXMLPath").equals(specId)) {
								specLineNr = specEle.getLineNr();
								specText = specEle.getAttribute("txt");
								break loopField;
							}
						}
					}
					if(-1!=specLineNr){
						LineNumberReader reader = new LineNumberReader(new InputStreamReader(getResources().getInputStream(SPEC_FILE_NAME)));
						StringBuilder xmlBuilder = new StringBuilder();
						String lineStr = null;
						for(int line=1;null!=(lineStr=reader.readLine());line++){
							if(line!=specLineNr){
								xmlBuilder.append(lineStr);								
							}else{
								...
								xmlBuilder.append("<spec txt=\"").append(specText).append("\" id=\"selectAppContextXMLPanel.appContextXMLPath\">");
								for (int index = 0; index < appContextXmlArr.length;) {
									File appContextXml = appContextXmlArr[index];
									xmlBuilder.append("<choice txt=\"").append(appContextXml.getName()).append("\"");
									if (index == 0) {
										xmlBuilder.append(" set=\"true\"");
									}
									xmlBuilder.append(" value=\"").append(appContextXml.getName()).append("\"");
									xmlBuilder.append(" id=\"appContextXMLPath.label." + (++index)).append("\"/>");
								}
								xmlBuilder.append("</spec>");
							}
						}
						StringReader xmlrReader = new StringReader(xmlBuilder.toString());
						ReaderInputStream in = new ReaderInputStream(xmlrReader);
						data = parser.parse(in);
						if (data.hasChildren()) {
							specElements = data.getChildrenNamed(NODE_ID);
							for (IXMLElement specEle : specElements) {
								userInputPanelSpecId = specEle.getAttribute(PANEL_IDENTIFIER);
								if (((userInputPanelSpecId != null) && (panelid != null) && (panelid.equals(userInputPanelSpecId)))) {
									spec = specEle;
									// close the stream
									in.close();
	 ...
	 * 
	 * @param spec
	 *            a <code>IXMLElement</code> containing the specification for
	 *            the title.
	 */
	/*--------------------------------------------------------------------------*/
	private void addTitle(IXMLElement spec) {
		...
		if (title != null) {
			...
			try {
				...
				logger.log(Level.WARNING, "Icon " + icon + " not found in icon list: " + e.getMessage(), e);
	...
	private void addRuleField(IXMLElement spec) {
		...
		// ----------------------------------------------------
		if (hasParams) {
			field = new RuleInputField(layout, set, separator, validator, validateParamMap, processor, resultFormat, getToolkit(), this.installData);
		} else {
			field = new RuleInputField(layout, set, separator, validator, processor, resultFormat, getToolkit(), this.installData);
	...
	/**
	 * Reads the installDataGUI from the rule input field and sets the
	 * associated variable.
	 * 
	 * @param field
	 *            the object array that holds the details of the field.
	 * @return <code>true</code> if there was no problem reading the
	 *         installDataGUI or if there was an irrecovarable problem. If there
	 *         was a problem that can be corrected by the operator, an error
	...
	private boolean readRuleField(UIElement field) {
		...
		entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, ruleField.getText()));
		return (true);
	}
	...
	private void addTextField(IXMLElement spec) {
		...
		// uiElements
		// .add(new Object[] { null, FIELD_LABEL, null, constraints, label,
		// forPacks, forOs});
		...
		// uiElements.add(new Object[] { null, TEXT_FIELD, variable,
		// constraints2, inputField,
	...
	/**
	 * Reads installDataGUI from the text field and sets the associated
	 * variable.
	 * 
	 * @param field
	 *            the object array that holds the details of the field.
	...
	private boolean readTextField(UIElement field) {
		...
		entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, value));
		return (true);
	}
	...
	private void addComboBox(IXMLElement spec) {
		...
		IXMLElement element = spec.getFirstChildNamed(SPEC);
		String variable = spec.getAttribute(VARIABLE);
		SelectAppContextXMLPanel.TextValuePair listItem = null;
			...
			for (int i = 0; i < choices.size(); i++) {
				...
				if (processorClass != null && !"".equals(processorClass)) {
					...
					while (tokenizer.hasMoreTokens()) {
						...
						listItem = new SelectAppContextXMLPanel.TextValuePair(token, token);
					...
					listItem = new SelectAppContextXMLPanel.TextValuePair(getText(choices.get(i)), value);
					field.addItem(listItem);
					if (userinput) {
						// is the current value identical to the value
		...
		// uiElements.add(new Object[] { null, COMBO_FIELD, variable,
		// constraints2, field, forPacks,
		// forOs});
	}
	...
	private boolean readComboBox(UIElement field) {
		...
		try {
			...
			value = ((SelectAppContextXMLPanel.TextValuePair) comboBox.getSelectedItem()).getValue();
		} catch (Throwable exception) {
			return true;
		}
		if ((variable == null) || (value == null)) {
	...
	private void addRadioButton(IXMLElement spec) {
		...
		if (element != null) {
			...
			for (IXMLElement choiceElement : choices) {
				...
				// uiElements.add(new Object[] { null, RADIO_FIELD, variable,
				// constraints, choice,
	...
	private void addPasswordField(IXMLElement spec) {
		...
		if (element != null) {
			...
			for (IXMLElement fieldSpec : inputs) {
				...
				// uiElements.add(new Object[] { null, FIELD_LABEL, null,
				// constraints, label,
				// forPacks, forOs});
	...
	private boolean readPasswordField(UIElement field) {
		...
		if (!success) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (!success) {
					JOptionPane.showMessageDialog(parent, group.getValidatorMessage(i), parent.getMessages().get("UserInputPanel.error.caption"), JOptionPane.WARNING_MESSAGE);
					break;
				}
			}
		}
		...
		if (success) {
			...
			entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, group.getPassword()));
		}
		return success;
	}
	...
	private void addCheckBox(IXMLElement spec) {
		...
		// uiElements.add(new Object[] { null, CHECK_FIELD, variable,
	...
	private boolean readCheckBox(UIElement field) {
		...
		if (box.isSelected()) {
			...
			entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, trueValue));
		} else {
			logger.fine("Not selected, setting " + variable + " to " + falseValue);
			this.installData.setVariable(variable, falseValue);
			entries.add(new SelectAppContextXMLPanel.TextValuePair(variable, falseValue));
		}
	...
	private void addSearch(IXMLElement spec) {
		...
		if (element != null) {
			...
			search_type = SelectAppContextXMLPanel.SearchField.TYPE_FILE;
			...
			if (search_type_str != null) {
				if (search_type_str.equals(SEARCH_FILE)) {
					...
					search_type = SelectAppContextXMLPanel.SearchField.TYPE_DIRECTORY;
				}
			}
			...
			if (result_type_str == null) {
				...
				result_type = SelectAppContextXMLPanel.SearchField.RESULT_FILE;
			} else if (result_type_str.equals(SEARCH_DIRECTORY)) {
				result_type = SelectAppContextXMLPanel.SearchField.RESULT_DIRECTORY;
			} else if (result_type_str.equals(SEARCH_PARENTDIR)) {
				result_type = SelectAppContextXMLPanel.SearchField.RESULT_PARENTDIR;
			} else {
				return;
			}
		...
		// uiElements.add(new Object[] { null, FIELD_LABEL, null,
		// westconstraint1, label, forPacks,
		// forOs});
		...
		if ((filename != null) && (filename.length() > 0)) {
			tooltiptext.append(MessageFormat.format(parent.getMessages().get("UserInputPanel.search.location"), new Object[] { new String[] { filename } }));
		}
		...
		// uiElements.add(new Object[] { null, SEARCH_FIELD, variable,
		// eastconstraint1, combobox,
		// forPacks, forOs});

		JPanel buttonPanel = new JPanel();
		buttonPanel.setLayout(new com.izforge.izpack.gui.FlowLayout(com.izforge.izpack.gui.FlowLayout.LEADING));

		JButton autodetectButton = ButtonFactory.createButton(getString("UserInputPanel.search.autodetect"), installData.buttonsHColor);
		...
		JButton browseButton = ButtonFactory.createButton(getString("UserInputPanel.search.browse"), installData.buttonsHColor);
		...
		// uiElements.add(new Object[] { null, SEARCH_BUTTON_FIELD, null,
		// eastonlyconstraint,
		// buttonPanel, forPacks, forOs});

		searchFields.add(new SelectAppContextXMLPanel.SearchField(filename, check_filename, parent, combobox, autodetectButton, browseButton, search_type, result_type));
	}
	 ...
	 * 
	 * @param spec
	 *            a <code>IXMLElement</code> containing other specifications. At
	 *            present this information is not used but might be in future
	 *            versions.
	 */
	/*--------------------------------------------------------------------------*/
	private void addSpace(IXMLElement spec) {
		...
		// uiElements
		// .add(new Object[] { null, SPACE_FIELD, null, constraints, panel,
		// forPacks, forOs});
	}
	 ...
	 * 
	 * @param spec
	 *            a <code>IXMLElement</code> containing additional
	 *            specifications.
	 */
	/*--------------------------------------------------------------------------*/
	private void addDivider(IXMLElement spec) {
		...
		// uiElements.add(new Object[] { null, DIVIDER_FIELD, null, constraints,
		// panel, forPacks,
		// forOs});
	}
	 ...
	 */
	/*--------------------------------------------------------------------------*/
	private void addDescription(IXMLElement spec, List<IXMLElement> forPacks, List<IXMLElement> forOs) {
		String description;
		TwoColumnConstraints constraints = new TwoColumnConstraints();
		constraints.position = TwoColumnConstraints.BOTH;
		constraints.stretch = true;
		...
		if (spec != null) {
			...
			if (description != null) {
				// String alignment = spec.getAttribute(ALIGNMENT);
				// FIX needed: where do we use this variable at all? i dont
				// think so...
				...
				// If html tags are present enable html rendering, otherwise the
				// JTextPane
				...
				// uiElements.add(new Object[] { null, DESCRIPTION, null,
				// constraints, label,
				// forPacks, forOs});
			}
		}
	 ...
	 * 
	 * @param element
	 *            the <code>IXMLElement</code> to search for the attribute.
	 * @param attribute
	 *            the attribute to search for
	 * @param defaultValue
	 *            the default value to use if the attribute does not exist or a
	 *            illegal value was discovered.
	 * @return <code>true</code> if the attribute is found and the value equals
	 *         the the constant <code>TRUE</code>. <<code> if the
	 *         attribute is <code>FALSE</code>. In all other cases the default
	 *         value is returned.
	 */
	/*--------------------------------------------------------------------------*/
	 ...
	 */
	/*--------------------------------------------------------------------------*/
	// private int getInt(IXMLElement element, String attribute, int
	// defaultValue)
	// {
	...
	/**
	 * Extracts the text from an <code>IXMLElement</code>. The text must be
	 * defined in the resource file under the key defined in the <code>id</code>
	 * attribute or as value of the attribute <code>txt</code>.
	 * 
	 * @param element
	 *            the <code>IXMLElement</code> from which to extract the text.
	 * @return The text defined in the <code>IXMLElement</code>. If no text can
	 *         be located, <code>null</code> is returned.
	 */
	/*--------------------------------------------------------------------------*/
	...
	/**
	 * Retreives the alignment setting for the <code>IXMLElement</code>. The
	 ...
	 * 
	 * @param element
	 *            the <code>IXMLElement</code> from which to extract the
	 *            alignment setting.
	 * @return the alignement setting for the <code>IXMLElement</code>. The
	 ...
	 * executed. The configuration for this feature is: <br/>
	 * &lt;os family="unix"/&gt; <br>
	 * <br>
	 * <b>Note:</b><br>
	 ...
	 * 
	 * @param os
	 *            The <code>Vector</code> of <code>String</code>s. containing
	 *            the os names
	 ...
	 * 
	 * @param packs
	 *            a <code>Vector</code> of <code>String</code>s. Each of the
	 *            strings denotes a pack for which an item should be created if
	 ...
	 * GUIInstallData.selectedPacks. This assumes that this panel is presented
	 * to the user AFTER the PacksPanel.
	 * ----------------------------------------
	 * ----------------------------------
	 */
	 ...
	 * system. This is a helper class to manage all installDataGUI belonging to
	 * a search field.
	 */
	/*---------------------------------------------------------------------------*/
	...
	private class SearchField implements ActionListener {
		 ...
		 * @param combobox
		 *            the <code>JComboBox</code> holding the list of choices; it
		 *            should be editable and contain only Strings
		 * @param autobutton
		...
		public SearchField(String filename, String checkFilename, InstallerFrame parent, JComboBox combobox, JButton autobutton, JButton browsebutton, int search_type, int result_type) {
			...
			((JTextField) this.pathComboBox.getEditor().getEditorComponent()).getDocument().addDocumentListener(new DocumentListener() {
				...
				private void checkNextButtonState() {
					javax.swing.text.Document doc = ((JTextField) pathComboBox.getEditor().getEditorComponent()).getDocument();
					try {
		...
		private boolean pathMatches(String path) {
			if (path != null) { // Make sure, path is not null
				...
				if (file.exists()) {
					...
					if (((this.searchType == TYPE_DIRECTORY) && (file.isDirectory())) || ((this.searchType == TYPE_FILE) && (file.isFile()))) {
		...
		public boolean autodetect() {
			...
			/*
			 * Check if the user has entered installDataGUI into the ComboBox
			 * and add it to the Itemlist
			 */
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource() == this.autodetectButton) {
				if (!autodetect()) {
					showMessageDialog(parent, "UserInputPanel.search.autodetect.failed.message", "UserInputPanel.search.autodetect.failed.caption", JOptionPane.WARNING_MESSAGE);
				}
				...
				if (result == JFileChooser.APPROVE_OPTION) {
					...
					if (this.resultType != TYPE_FILE && !this.pathMatches(selectedFile.getAbsolutePath())) {
						showMessageDialog(parent, "UserInputPanel.search.wrongselection.message", "UserInputPanel.search.wrongselection.caption", JOptionPane.WARNING_MESSAGE);
		...
		for (IXMLElement variable : variables) {
			...
			if (matcher.matchesCurrentPlatform(osList)) {
				if (vname == null) {
					...
					// for save this variable to be used later by Automation
					// Helper
	 ...
	 *            The parent frame.
	 * @param message
	 *            The message to print out in dialog box.
	 * @param caption
	 *            The caption of dialog box.
	 * @param messageType
	 *            The message type (JOptionPane.*_MESSAGE)
	 */
	/*--------------------------------------------------------------------------*/
	 ...
	 * 
	 * @param text
	 *            the text to perform replacement on. May be {@code null}
